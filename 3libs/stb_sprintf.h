...
static STBSP__ASAN stbsp__uint32 stbsp__strlen_limited(char const *s,
  ...
  for (;;) {
    ...
    if (!limit || *sn == 0) return (stbsp__uint32)(sn - s);
  ...
  if (callback) {                               \
    ...
    for (;;) {
      ...
      for (;;) {
        ...
        if (callback)
          if ((STB_SPRINTF_MIN - (int)(bf - buf)) < 4) goto schk1;
#ifdef STB_SPRINTF_NOUNALIGNED
...
#define STBSP__NUMSZ 512
      ...
      dofloatfromg:
        ...
        if (dp <= 0) {
          ...
          *s++ = '0';
          if (pr) *s++ = stbsp__period;
          n = -dp;
          if ((stbsp__int32)n > pr) n = pr;
          i = n;
          while (i) {
            if ((((stbsp__uintptr)s) & 3) == 0) break;
            *s++ = '0';
            --i;
          }
          while (i >= 4) {
          ...
          cs =
              (fl & STBSP__TRIPLET_COMMA) ? ((600 - (stbsp__uint32)dp) % 3) : 0;
        ...
        if (fl & STBSP__METRIC_SUFFIX) {
          char idx;
          idx = 1;
          if (fl & STBSP__METRIC_NOSPACE) idx = 0;
          tail[0] = idx;
          tail[1] = ' ';
          {
            if (fl >> 24) {
      ...
      radixnum:
        ...
        cs = (stbsp__uint32)((num + STBSP__NUMSZ) - s) +
             ((((l >> 4) & 15)) << 24);
        ...
        if (fw + pr) {
          ...
          if ((fl & STBSP__LEFTJUST) == 0)
            while (fw > 0) {
              ...
              fw -= i;
              while (i) {
                if ((((stbsp__uintptr)bf) & 3) == 0) break;
                *bf++ = ' ';
                --i;
              }
              while (i >= 4) {
...
static stbsp__int32 stbsp__real_to_str(char const **start, stbsp__uint32 *len,
  ...
  if ((frac_digits < 24)) {
    stbsp__uint32 dg = 1;
    if ((stbsp__uint64)bits >= stbsp__powten[9]) dg = 10;
