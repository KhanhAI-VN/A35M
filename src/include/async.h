...
static void run_inference_task(uv_work_t *req) {
  ...
  if (!cache->model) {
    Arena arena = {0};
    size_t max_model_size = 1024 * 1024 * 2;
    uint8_t *l_model_buf = arena_alloc(&arena, max_model_size);

    if (l_model_buf) {
      int m_len =
          download_model_from_github(cname, l_model_buf, max_model_size);
      if (m_len > 0) {
  ...
  cache->last_res.last_price =
      (float)cache->klines[task->kline_count - 1].close;
  float yesterday_close = (float)cache->klines[task->kline_count - 2].close;
  cache->last_res.change_pct =
      (yesterday_close > 1e-9)
          ? ((cache->last_res.last_price - yesterday_close) / yesterday_close) *
                100.0f
          : 0;
  ...
  while ((message = curl_multi_info_read(curl_handle, &pending))) {
    switch (message->msg) {
      case CURLMSG_DONE: {
        ...
        if (message->data.result == CURLE_OK && http_code == 200 &&
            sdslen(ctx->response) > 0) {
          yyjson_doc *doc =
              yyjson_read(ctx->response, sdslen(ctx->response), 0);
          if (doc) {
            ...
            if (n >= SEQ_LEN + 2) {
              ...
              uv_queue_work(loop, &task->req, run_inference_task,
                            on_inference_done);
            } else {
              free(task);
            }
...
static size_t async_write_cb(void *contents, size_t size, size_t nmemb,
                             void *userp) {
...
static void add_download(const char *coin) {
  ...
  stbsp_snprintf(ctx->url, sizeof(ctx->url),
                 "https://api.binance.com/api/v3/"
                 "klines?symbol=%sUSDT&interval=1d&limit=%d",
                 coin, SEQ_LEN + 2);
